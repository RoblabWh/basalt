...
* `optimize` runs an iteration of the optimization and visualizes the result. You should press this button until the error printed in the console output stops decreasing and the optimization converges. Alternatively, you can use the `opt_until_converge` checkbox that will run the optimization until it converges automatically. Note that frames whose initial pose estimation failed are excluded from the optimization. After the optimization converges, press `init_cam_poses`, `init_opt` and `optimize` again. The poses are then re-estimated with the optimized calibration, which typically recovers almost all frames.
